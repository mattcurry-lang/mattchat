  ...
  return (
    ...
    >
      ...
      {DRAW_TOOLS.map(({ id, label, Icon }) => (
        <ToolButton key={id} active={tool === id} onClick={() => onToolChange(id)} title={label}>
          <Icon size={17} />
        </ToolButton>
      ))}
      ...
      <div style={{ position: 'relative' }}>
        <ToolButton active={isShapeActive} onClick={() => setShowShapes(v => !v)} title="Shapes">
          <IconShapes size={17} />
        </ToolButton>
        {showShapes && (
          <div style={{
            position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: '#1e1e2e',
            border: '1px solid rgba(167,139,250,0.25)', borderRadius: 12, padding: 6,
            display: 'flex', flexDirection: 'column', gap: 2, zIndex: 30, minWidth: 130,
            boxShadow: '0 8px 30px rgba(0,0,0,0.4)',
          }}>
            {SHAPE_TOOLS.map(s => (
              <button
                key={s.id}
                onClick={() => { onToolChange(s.id); setShowShapes(false) }}
                style={{
                  display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderRadius: 8,
                  background: tool === s.id ? 'rgba(167,139,250,0.15)' : 'none', border: 'none',
                  color: tool === s.id ? '#c4b5fd' : 'rgba(255,255,255,0.75)', fontSize: 12.5, fontWeight: 600,
      ...
      <div style={{ position: 'relative' }}>
        ...
        />
        {showColorPicker && (
          <ColorPicker color={color} onChange={onColorChange} onClose={() => setShowColorPicker(false)} />
        )}
      </div>
      ...
      <ToolButton active={false} onClick={onUndo} title="Undo (Ctrl+Z)">
        <IconUndo size={17} style={{ opacity: canUndo ? 1 : 0.3 }} />
      </ToolButton>
      <ToolButton active={false} onClick={onRedo} title="Redo (Ctrl+Shift+Z)">
        <IconRedo size={17} style={{ opacity: canRedo ? 1 : 0.3 }} />
      </ToolButton>
      <ToolButton active={false} onClick={onClear} title="Clear canvas">
        <IconTrash size={17} />
      </ToolButton>
      ...
      <ToolButton active={false} onClick={onToggleFullscreen} title={isFullscreen ? 'Exit fullscreen' : 'Fullscreen'}>
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 10, color: '#f87171', width: 34, height: 34,
