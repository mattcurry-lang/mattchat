...
import { IconX, IconEyeOff, IconSparkle, IconBell,IconSettings } from '../Icons'
...
// Replace the return statement's outer structure with this:
return (
  <div className="cycle-dash-root" style={{
    ...
    <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '16px 20px', gap: 8 }}>
      <button onClick={() => setShowDataSettings(true)} title="Edit cycle info" style={iconBtnStyle}><IconSettings size={15} /></button>
    ...
    <div className="cycle-dash-container">
      {settings?.hide_cycle && (
        <div className="cycle-dash-span-all" style={{ background: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: 12, padding: '10px 14px', fontSize: 12.5, color: '#86efac', display: 'flex', alignItems: 'center', gap: 8 }}>
          <IconEyeOff size={14} /> Privacy mode is on — sharing is paused. Your data is safe and untouched.
        </div>
      )}

      {!hasData ? (
        <div className="cycle-dash-span-all" style={{ textAlign: 'center', padding: '40px 20px', background: 'rgba(255,255,255,0.04)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <>
          {/* LEFT COLUMN — the ring, phase card, streak */}
          <div className="cycle-dash-left" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{
            ...
            {phase && (
              <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 18, padding: '16px 18px' }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: '#c4b5fd', marginBottom: 6 }}>{phase?.label || 'Cycle Update'}</div>
                <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', lineHeight: 1.6 }}>{phase?.blurb || ''}</div>
              </div>
            )}

            {checkinStreak > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, alignSelf: 'flex-start', background: 'rgba(167,139,250,0.12)', border: '1px solid rgba(167,139,250,0.3)', borderRadius: 20, padding: '5px 12px', fontSize: 12, fontWeight: 700, color: '#c4b5fd' }}>
                🔥 {checkinStreak}-day check-in streak
              </div>
            )}
          </div>

          {/* RIGHT COLUMN — today for you, stats, actions */}
          <div className="cycle-dash-right" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <TodayForYouCard phase={cycleInfo.phase} recentDailyLogs={recentDailyLogs} onOpenWellness={onOpenWellness} />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <StatCard label="Average cycle" value={stats?.average ? `${stats.average} days` : 'Need more data'} />
            </div>

            <button onClick={onOpenCheckin} className="btn-primary" style={{ padding: 14, fontSize: 14, borderRadius: 16 }}>
              How are you feeling today?
            </button>

            <button onClick={toggleHideCycle} disabled={hiding} style={{
    ...
    <style>{`
      ...
      @media (min-width: 1024px) {
        ...
        .cycle-dash-span-all { grid-column: 1 / -1; }
      }
    `}</style>
  </div>
)
