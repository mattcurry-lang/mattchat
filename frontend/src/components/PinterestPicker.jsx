...
export default function PinterestPicker({ session, userId, preference, onPicked, onBack }) {
  ...
  // If the OAuth redirect completes while this modal is open, re-check.
  useEffect(() => {
  ...
  const handleConnect = async () => {
    setConnecting(true)
    try { await connectPinterest(session) } catch (e) { setError(e.message); setConnecting(false) }
  }
      ...
      {phase === 'pins' && (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button className="btn-ghost" style={{ padding: '4px 10px' }} onClick={() => setPhase('boards')}>← Boards</button>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{activeBoard?.name}</div>
          </div>
          {error && <div className="modal-error">{error}</div>}
