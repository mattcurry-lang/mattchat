        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 13, color: 'var(--dark-text-2)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <IconCheckSquare size={13} /> Completed: <strong style={{ color: 'var(--dark-text)' }}>{report.completedCount}</strong>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <IconClock size={13} /> Hours studied: <strong style={{ color: 'var(--dark-text)' }}>{report.hoursStudied}</strong>
            </div>
            {report.completionRate !== null && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <IconChart size={13} /> Completion rate: <strong style={{ color: 'var(--dark-text)' }}>{report.completionRate}%</strong>
              </div>
            )}
            {report.mostProductiveDay && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <IconStar size={13} filled /> Most productive day: <strong style={{ color: 'var(--dark-text)' }}>{report.mostProductiveDay}</strong>
              </div>
            )}
            {report.missedDeadlines > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#f87171' }}>
                <IconAlertTriangle size={13} /> Missed deadlines: {report.missedDeadlines}
              </div>
            )}
              ...
              <div style={{ background: 'rgba(167,139,250,0.08)', border: '1px solid rgba(167,139,250,0.2)', borderRadius: 10, padding: 12 }}>
                {report.suggestions.map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: 4 }}>
                    <IconLightbulb size={13} style={{ marginTop: 1, flexShrink: 0 }} />
                    <span>{s}</span>
                  </div>
                ))}
              </div>
